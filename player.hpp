...
class Player {
    ...
    bool validateCastleMove(Board& board, std::string const& start,
                            std::string const& end);
    /* Check if there is any opponent's piece that has a valid move to position
     * coor. */
    bool underAttack(Board& board, Coor const& coor) const;
    /* Check the validity of the move without modifying the board,
     * return corresponding error code. */
    InvalidMove validateMove(Board& board, std::string const& start,
                             std::string const& end);
    /* Assume the move is valid and change the board and kingPos as appropriate.
     * If not hypothetical, mark the piece as moved. Return a pointer to the
     * captured piece if there is a capture otherwise nullptr.  */
    std::unique_ptr<Piece> makeMove(Board& board, std::string const& start,
                                    std::string const& end,
     ...
     */
    void moveBack(Board& board, std::string const& start,
                  std::string const& end,
...
public:
    ...
    /* Return InvalidMove::NO_ERROR if valid move then make the change on board,
     * else return the err_code. */
    InvalidMove submitMove(Board& board, std::string const& start,
                           std::string const& end);
     ...
     * 2. after makeMove, update whether the opponent is in check. */
