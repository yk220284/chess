...
Player* const& Player::getOpponent() const { return opponent; }
bool const& Player::checkStatus() const { return inCheck; }
bool& Player::checkStatus() { return inCheck; }
Color const& Player::getColor() const { return color; }
/* ----validate Castling---- */
...
/* Given the castle movement of the king, return the start and end location of
 * the corresponding rook. */
std::pair<Coor, Coor> locateRook(std::string const& kingStart,
                                 std::string const& kingEnd)
{
...
bool Player::validateCastleMove(Board& board, std::string const& start,
                                std::string const& end)
{
    ...
    // distance is the number of square between two pieces.
    ...
    // 4. Note it's impossible to castle and resulting yourself in check. So no
    // need to make a hypothetical move.
    return true;
}
/* ----validateMove---- */
InvalidMove Player::validateMove(Board& board, std::string const& start,
                                 std::string const& end)
{
    ...
    auto occupiedPos =
        std::find_if(path->begin(), path->end(), [&board](auto const& pos) {
    ...
    // 5c. Moved Pawn diagonally without capturing any piece.
    if (!capturedPiece && path->size() == 0 &&
        board[end]->getPieceType() == PieceType::pawn) {
        moveBack(board, start, end, capturedPiece); // Undo the damage.
        return InvalidMove::ILLEGAL_PIECE_MOVE;
    }
    // 8a. Captured opponent's king, don't need to check whether I am in check
    // anymore.
...
/* ---- print error message ---- */
void printError(InvalidMove invalidMove, Board const& board,
                std::string const& start, std::string const end)
{
    ...
    case InvalidMove::INVALID_POS:
        std::cerr << start << " or " << end
                  << " is not a valid position on board\n";
        break;
    case InvalidMove::NO_PIECE:
        std::cerr << "There is no piece at position " << start << "!\n";
        break;
    case InvalidMove::OPPONENT_PIECE:
        std::cerr << "It is not " << board[start]->getColor()
                  << "’s turn to move!\n";
        break;
    case InvalidMove::STATIONARY_MOVE:
        std::cerr << "You didn't move your piece (the start position is the "
    ...
    case InvalidMove::BLOCK:
        std::cerr << board[start]->getColor() << "'s "
                  << board[start]->getPieceType() << " cannot move from "
                  << start << " to " << end
...
/* ----submitMove---- */
InvalidMove Player::submitMove(Board& board, std::string const& start,
                               std::string const& end)
{
    ...
    if (err_code == InvalidMove::NO_ERROR) {
        ...
        inCheck = false;
        std::cout << color << "'s " << board[start]->getPieceType()
                  << " moves from " << start << " to " << end;
        auto capturedPiece = makeMove(board, start, end, false);
        if (capturedPiece) {
            std::cout << " taking " << capturedPiece->getColor() << "'s "
                      << capturedPiece->getPieceType();
        }
        std::cout << std::endl;
        // check if it's a castling move, if so, move the corresponding Rook.
        if (isCastling(start, end)) {
            auto p = locateRook(start, end);
            auto const& rookStartCoor = p.first;
            auto const& rookEndCoor = p.second;
            std::cout << color << "'s " << board[rookStartCoor]->getPieceType()
                      << " moves from " << rookStartCoor << " to "
                      << rookEndCoor << " finish castling.\n";
...
/* ----makeMove---- */
std::unique_ptr<Piece> Player::makeMove(Board& board, std::string const& start,
                                        std::string const& end,
                                        bool hypothetical)
{
    if (!hypothetical) {
...
/* ----moveBack---- */
void Player::moveBack(Board& board, std::string const& start,
                      std::string const& end,
    ...
    for (int x = 0; x < 8; x++) {
        for (int y = 0; y < 8; y++) {
            if (board[Coor(x, y)] &&
                ...
                opponent->validateMove(board, Coor(x, y).str(), coor.str()) ==
    ...
    for (int x = 0; x < 8; x++) {
        for (int y = 0; y < 8; y++) {
            ...
            if (piece && piece->getColor() == color) {
                auto potentialEnds =
                    piece->potentialEndPositions(Coor(x, y).str());
                for (auto const& end : potentialEnds) {
                    // Exists at least 1 valid move for me.
                    if (validateMove(board, Coor(x, y).str(), end.str()) ==
                        InvalidMove::NO_ERROR) {
                        return false;
                    }
                }
