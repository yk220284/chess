...
Piece::Path Pawn::findPath(std::string const &start, std::string const &end)
{
    ...
    // Capture other piece.
    if (abs(endCoor.x - startCoor.x) == 1 && endCoor.y - startCoor.y == advanceDirection)
    {
        {
            path = std::make_unique<std::vector<Coor>>();
        }
    ...
    else if (endCoor.x == startCoor.x)
    {
        ...
        {
            path = std::make_unique<std::vector<Coor>>();
            path->emplace_back(endCoor); // Need to check if there is any piece at endCoor.
        }
        else if (endCoor.y - startCoor.y == 2 * advanceDirection && !hasMoved())
        {
            path = std::make_unique<std::vector<Coor>>();
            path->emplace_back(startCoor.x, startCoor.y + advanceDirection);
            path->emplace_back(endCoor); // Need to check if there is any piece in the path and at endCoor.
        }
    }
    return path;
}
...
std::vector<Coor> const Knight::directions =
    {Coor(1, 2), Coor(2, 1), Coor(2, -1), Coor(1, -2), Coor(-1, -2), Coor(-2, -1), Coor(-2, 1), Coor(-1, 2)};
...
// Retern all possible end positions.
std::vector<Coor> positionHelper(std::string const &start, std::vector<Coor> const &directions)
{
    std::vector<Coor> positions;
    Coor const startCoor{start};
    for (auto const &direction : directions)
...
/*--------------------------Bishop--------------------------*/
std::vector<Coor> const Bishop::directions = {Coor(1, 1), Coor(1, -1), Coor(-1, -1), Coor(-1, 1)};
